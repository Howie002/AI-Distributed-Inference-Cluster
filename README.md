...
| `boot.sh` / `boot.meta.json` | Fleet-standard headless bring-up (key `llm-proxy`) — starts the control agent, then has it regenerate the proxy config and start LiteLLM. Discovered by the fleet boot orchestrator and watchdog |
| `litellm/cluster_config.yaml` | **The live LiteLLM proxy config — generated** by the control agent from the instances running across the cluster. Never hand-edit; rewritten on launch/stop and reconciled every 60s |
| `litellm_config.yaml` | **DEPRECATED, emptied.** Legacy single-box routing (nemotron on localhost); superseded by `litellm/cluster_config.yaml` |
| `start_inference_stack.sh` | **DEPRECATED**, refuses to run without `VLLM_LEGACY_STACK=1` — legacy single-box path that would serve dead models on `:4000` |
