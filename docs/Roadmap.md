...
- [ ] **Evaluate IndexTTS 2.5 without changing production Voicebox.** Source: https://huggingface.co/IndexTeam/IndexTTS-2.5. Treat it as an unverified candidate, not an adopted stack component.
  - [ ] Verify model provenance, license, and commercial-use terms.
  - [ ] Confirm local/offline deployment, model size, hardware/VRAM requirements, and compatibility with the Foundation's Blackwell fleet.
  - [ ] Benchmark cold start, warm latency, concurrent latency, audio quality, voice consistency, controllability, and cloning/privacy implications against the current Voicebox engines.
  - [ ] Determine API/sidecar fit and whether it could be integrated behind Voicebox or must remain a separate service.
  - [ ] Define a rollback path and do not alter the Voicebox failover proxy or production consumers during evaluation.

## New from 2026-08-26 - the dashboard is servable over HTTPS; launches are guarded (#242/#212)

- [x] **Agent calls moved same-origin.** `agentBase()` returned `http://<node ip>:5000`, a direct browser call over the AI VLAN — the single reason this dashboard could not be served over HTTPS, and why the Foundation dashboard's `/InferenceCluster` tile 404ed. Now `/api/agent/<ip>`, a route handler that fans out server-side. All ~48 agent calls funnel through that one function, so no component changed. The old `next.config.mjs` rewrite is gone: it pointed at a single `AGENT_URL` and could only ever reach the master.
...
- **Average GPU utilization (trailing 24h)** - fleet-wide headline stat on the GPU Cluster header and an "Avg GPU util" tile on the Analytics totals strip; averages `util_pct_avg` across every node/GPU from the existing `metrics("24h","1h")` buckets (no new instrumentation), always 24h regardless of the range picker; shipped 2026-08-19 (`c65ef41`)
